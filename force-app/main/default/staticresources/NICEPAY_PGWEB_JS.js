﻿(function(w){
	...
	/**
	* @description       : nicepay-gw.js 수정 내역
		[S&I] - sungho.jo@mz.co.kr
		salesforce에서 사용할 수 있도록, 불필요한 내용 수정
		[modified] 된 부분으로 수정된 내역을 찾을 수 있음
	...
	**/
		...
		try{
			...
			if(payForm.TransType) {
				if(payForm.TransType.value == "1"){
					console.log(' payForm.PayMethod : ', payForm.PayMethod)
	...
	NicePayStd.resultMessage = function(obj){
		...
		NicePayStd.resultSalesforce();
	}

	// [modified] Salesforce로 nice-pay-result CustomEvent로 JSON으로 리턴
	NicePayStd.resultSalesforce = () => {
		const obj = {
			"GoodsName" 			: merchantForm?.GoodsName?.value
			, "Amt" 				: merchantForm?.Amt?.value
			, "MID" 				: merchantForm?.MID?.value
			, "EdiDate"				: merchantForm?.EdiDate?.value
			, "Moid" 				: merchantForm?.Moid?.value
			, "SignData"			: merchantForm?.SignData?.value
			, "PayMethod"			: merchantForm?.PayMethod?.value
			, "ReqReserved"			: merchantForm?.ReqReserved?.value
			, "CharSet"				: merchantForm?.CharSet?.value
			, "VerifySType"			: merchantForm?.VerifySType?.value
			, "EncGoodsName"		: merchantForm?.EncGoodsName?.value
			, "EncBuyerName"		: merchantForm?.EncBuyerName?.value
			, "NpDirectYn"			: merchantForm?.NpDirectYn?.value
			, "NpDirectLayer"		: merchantForm?.NpDirectLayer?.value
			, "JsVer"				: merchantForm?.JsVer?.value
			, "NpSvcType"			: merchantForm?.NpSvcType?.value
			, "DeployedVer"			: merchantForm?.DeployedVer?.value
			, "DeployedDate"		: merchantForm?.DeployedDate?.value
			, "DeployedFileName"	: merchantForm?.DeployedFileName?.value
			, "AuthResultCode"		: merchantForm?.AuthResultCode?.value
			, "AuthResultMsg"		: merchantForm?.AuthResultMsg?.value
			, "AuthToken"			: merchantForm?.AuthToken?.value
			, "TxTid" 				: merchantForm?.TxTid?.value
			, "NextAppURL"			: merchantForm?.NextAppURL?.value
			, "NetCancelURL"		: merchantForm?.NetCancelURL?.value
			, "AuthFlg" 			: merchantForm?.AuthFlg?.value
			, "BuyerAuthNum"		: merchantForm?.BuyerAuthNum?.value
			, "ServiceIDC"			: merchantForm?.ServiceIDC?.value
			, "Signature"			: merchantForm?.Signature?.value
		};
		const nicePayResultCustomEvent = new CustomEvent('nice-pay-result', { 
			bubbles: true,
			detail: obj
		});
		dispatchEvent(nicePayResultCustomEvent);
	}
	...
	// [modified] window.nicepay를 통해 호출할 수 있도록 추가
	w.nicepay = {
		"goPay" : goPay
	};
})(window);
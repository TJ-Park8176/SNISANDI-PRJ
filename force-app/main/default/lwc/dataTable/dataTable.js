...
//import getAccountSubjects from '@salesforce/apex/DataTableController.getAccountSubjects';
//import getCheckWbsCc from '@salesforce/apex/DataTableController.getCheckWbsCc'
import getAmountInfo from '@salesforce/apex/DataTableController.getAmountInfo';
import getSumAmount from '@salesforce/apex/DataTableController.getSumAmount';
import getWBSList from '@salesforce/apex/DataTableController.getWBSList';
import getCSS from '@salesforce/apex/DataTableController.getCSS';


let TOTAL_TD_MONTHS = 75; //78; //13*N : 현장=78(N=6), 운영=39(N=3)
const SITE_COL_NUM = 6; //현장
const OPS_COL_NUM = 3;  //운영
...
export default class DataTable extends LightningElement {
    ...
    YorP;
    locType = '현장'; //초기값 '현장'
    //selectBoxLabel = 'WBS/CC'; //초기값 'WBS/CC'
    ...
    locTypeDisabled = true; //현장/운영 구분 토글 : 초기값-비활성화
    ...
    async connectedCallback() {
        ...
        Promise.all([
            ...
            //event
            //현장/운영 구분 토글버튼 클릭 이벤트
            const self = this;
            $(this.template.querySelectorAll('input[name="locType"]')).on('click', function() {
                console.log('before click : ' + self.locType);
                console.log('after click : ' + $(this).val());

                let currentValue = $(this).val();

                if(currentValue == '현장') {
                    TOTAL_TD_MONTHS = 3 + (12 * SITE_COL_NUM); //합계(3) + 월별칼럼개수
                } else if(currentValue == '운영') {
                    TOTAL_TD_MONTHS = 13 * OPS_COL_NUM;
                }

                if(self.locType != currentValue) { //다른거 클릭 시
                    self.locType = currentValue;
                    self.fetchAmountInfo();
                    /* 현장/운영 disable 삭제
                    if(self.loadToggle) {
                        self.fetchAmountInfo();
                    }
...
console.log(this.locType);
        ...
        } else if(this.locType == '운영') {
            colspanValue = OPS_COL_NUM;
        ...
        $(thead).empty().append(
            `<tr class="titleCol">
                ...
                <th colspan="3" style="min-width:500px;">연간 합계</th>
             </tr>`
         );
        ...
        titleColumInfos.forEach(col => {
            $(titleCol).append(`<th colspan="${colspanValue}">${col.Label}</th>`);
        });
        ...
        for(let i=0; i<13; i++) {
            numColumInfos.forEach((col,index) => {
                //운영 & 연간합계 : 실적 합계, 나머지 : 실적 추정
                $(subTitleCol).append(
                    `<th class="numColum">
                        ${(i == 0 && index == 1 && this.locType == '운영') ? '실적 합계(연간추정)' : col.Label}
                    </th>`
                );
                //$(subTitleCol).append(`<th class="numColum">${col.Label}</th>`);
            });
        }
        */
...
//23.05.30 dgchoi 연간합계 열 수정
        $(subTitleCol).append(
            `<th class="numColum">사업 계획</th>
             <th class="numColum">실적 합계(연간추정)</th>
             <th class="numColum">GAP</th>`
        );
        ...
        this.amountInfo.forEach(rec => {
            ...
            if(self.locType == '현장') {
                pList.forEach((p,index) => {
                    //23.05.30 dgchoi 연간합계 열 추가
                    if(index == 0) {
                        body += `<td class="amt" data-month="${p.CalMonth}">${p.PlanAmt}</td>
                                 <td class="amt" data-month="${p.CalMonth}">${p.PerformanceEstimate}</td>
                                 <td class="amt" data-month="${p.CalMonth}">${p.PerformanceEstimateGAP}</td>`;
                    } else {
                        body += `<td class="amt" data-month="${p.CalMonth}">${p.PlanAmt}</td>
                                 <td class="amt" data-month="${p.CalMonth}">${p.ActionPlanAmt}</td>
                                 <td class="amt" data-month="${p.CalMonth}">${p.ExpectPerformance}</td>
                                 <td class="amt" data-month="${p.CalMonth}">${p.Performance}</td>
                                 <td class="amt" data-month="${p.CalMonth}">${p.ExpectGAP}</td>
                                 <td class="amt" data-month="${p.CalMonth}">${p.PerformanceGAP}</td>`;
                    }
                });
            } else if(self.locType == '운영') {
                pList.forEach(p => {
                    body += `<td class="amt" data-month="${p.CalMonth}">${p.PlanAmt}</td>
                             <td class="amt" data-month="${p.CalMonth}">${p.PerformanceEstimate}</td>
                             <td class="amt" data-month="${p.CalMonth}">${p.PerformanceEstimateGAP}</td>`;
...
*/
    ...
    async fetchAmountInfo() {
        ...
        await getAmountInfo({
            ...
            console.log("fetchAmountInfo(" + this.wbsId + ", " + this.locType + ") return Success");
            console.log(JSON.parse(data));
            if(data.length > 0) {
                this.amountInfo = JSON.parse(data);
                this.wbsOrCc = this.amountInfo[0].WBS.WBSorCC;
    ...
    downloadCSVFile() {
        ...
        downloadElement.download = '손익계산서.xls';

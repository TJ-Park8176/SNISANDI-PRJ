...
import getCostList from '@salesforce/apex/ItemMngPlanController.getCostList';
import updateCostList from '@salesforce/apex/ItemMngPlanController.updateCostList';
import deleteCostList from '@salesforce/apex/ItemMngPlanController.deleteCostList';
import isAmountItem from '@salesforce/apex/ItemMngPlanController.isAmountItem';
...
const FIELDS = ['Itemmanagement_Plan__c.Id',
                ...
                'Itemmanagement_Plan__c.AccountSubjectSort__c'];

export default class CostPlanManageFormItem extends LightningElement {
    @api recordId;
    ...
    wiredRecord({error, data}) {
        if(error) {
            ...
            this.AccountSubjectsortField    = data.fields.AccountSubjectSort__c.value;
        }
    }
    ...
    async setCostList(data) {
        let sessionData = JSON.parse(sessionStorage.getItem('costPlanList_' + this.recordId));
        let parseData   = JSON.parse(data);

        console.log('sessionData:',sessionData);
        console.log('parseData:', parseData);
        if(parseData.length > 0) { //리턴O
            if(sessionData) { //세션O
                ...
                this.costList.forEach((cost, costIndex) => {
                    cost.data.forEach((e, i) => {
                        e.data.savedAmount = e.data.PlanPurchaseAmount__c || 0;
                    })
                });
            }
        } 
        ...
        //             data: {PlanPurchaseAmount__c: 0},
        //             cno:i,
        //             amountPrice: 0
        //         });
        //     }
        //     this.costList = this.costList.map((data, index) => {
        //         return {data, sno:index+1, subjectName:'계정 과목 선택', isSaved: false};
        //     });
        // }
    ...
    costTotalSum() {
        for(var num = 0; num < this.costList.length ; num++){
            ...
            for(var index = 0; index < 12 ; index++){
                this.costTotal += parseFloat(this.costList[num].data[index].data.PlanPurchaseAmount__c);
            }
    ...
    async updateCost() {
        await updateCostList({
            ...
            if(result.status == 'S') {
                sessionStorage.removeItem('costPlanList_'+this.recordId),
    ...
    handleBlur(event) {
        try {
            ...
            input_price.forEach((ele, index) => {
                ...
                this.costList[eIndex].data[index].data.PlanPurchaseAmount__c = parseFloat(ele.value);
            });
    ...
    setTotal() {
        ...
        this.costList.forEach(e => {
            for(var index = 0; index < 12 ; index++){
                ...
                this.costTotal += parseFloat(e.data[index].data.PlanPurchaseAmount__c);
            }
        });
    }
    */

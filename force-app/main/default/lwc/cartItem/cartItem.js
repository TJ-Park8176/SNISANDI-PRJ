...
import updateCartItemSelectedStatus from '@salesforce/apex/Sandi_CartItemController.updateCartItemSelectedStatus';
import updateCartItemSpecialOfferStatus from '@salesforce/apex/Sandi_CartItemController.updateCartItemSpecialOfferStatus';
import optionChangeModal from 'c/sandi_cartItemOptionChangeForm';
...
//const FIELDS = ['CartItem.IsSelected_SANDI__c'];
...
/*    *//**
     * @description Checked Checkbox if isSelected true
     *//*
    isSelectedItem = false;

    @wire(getRecord, { recordId: '$itemId', fields: FIELDS })
    wiredRecord({ error, data }) {
        try {
            if (error) {
                ...
                this.isSelectedItem = data.fields.IsSelected_SANDI__c.value;
            }
        } catch (err) {
            console.log("🚀 ~ err", err);
        }
    ...
    /**
     * 옵션 변경 버튼 활성화 여부 (Variation 상품일 경우 활성화)
     */
    isVariationItem;
    ...
    connectedCallback() {
        ...
        // 판매형 상품인지
        this.isSalesProduct = this.item.SalesType_SANDI__c === '판매형';
    }

    renderedCallback() {
        // report invalid quantities after rendering the item
        this.refs.quantitySelector?.reportValidity();
        if(this.item.ProductClass === 'Variation') this.isVariationItem = true; //옵션 변경
    ...
    handleSpecialOfferChanged(event) {
        console.log('handleSpecialOfferChanged() ---> ');
        updateCartItemSpecialOfferStatus({
            cartItemId    : this.item.id,
            selectedStatus: event.target.checked
        }).then((data) => {
            console.log('🚀 ~ data', data);
    ...
    handleModal(){
        optionChangeModal.open({
            size: 'small',
            description: '옵션 변경 모달',
            cartItemId: this.item.id,
        }).then((result)=>{
            // 모달 닫은 뒤 이벤트
        });
